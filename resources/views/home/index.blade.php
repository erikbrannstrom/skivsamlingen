...
<p>Till att börja med <a href="/account/register">registrerar</a> du dig. Det enda du behöver fylla i är ett <strong>användarnamn</strong> och ett <strong>lösenord</strong> och sen är det bara att sätta igång!</p>
...
@else
<h2>Hej {{ Auth::user()->name ?: Auth::user()->username }}!</h2>
...
<tr>
    ...
    <td>{{ $members['total'] > 0 ? round($total_recs / $members['total'], 1) : 0 }}</td>
</tr>
</table>
</div>
...
<ul class="donate">
  ...
  <p><a href="/news">Alla nyheter</a></p>
...
@foreach($latest_records as $record)
    <a href="/users/{{ $record->username }}">{{ $record->name }} - {{ $record->title }}</a><br />
@endforeach
</div>
